...
```mermaid
flowchart TD
    START(["🗣️ Wake word 'Hey Mycroft' (locale, offline)<br/>oppure click encoder"])
...
- **Attivazione**: **wake word locale "Hey Mycroft"** (microWakeWord/TFLite Micro, tutto
...
> **Wake word**: "Hey Mycroft" è un modello **pre-addestrato**. Dal 2 set 2026: prima era
> "Okay Nabu", ceduta al Balancing Robot perché una parola sola non può accendere due
> apparecchi nella stessa casa, e il robot è l'unico dei due che l'aveva già provata dal
> vivo. Gli altri modelli inglesi davano problemi (accento; "alexa" scattava su qualsiasi
> "-xa"), quindi il catalogo pronto è esaurito: una parola **CUSTOM** è l'unico passo che
> resta (training microWakeWord via Colab). Pipeline completa in [`WAKEWORD.md`](WAKEWORD.md).
...
src/
  ...
  wake_model.h       # modello wake INT8 (g_wake_model): "hey_mycroft". Sostituibile (drop-in)
